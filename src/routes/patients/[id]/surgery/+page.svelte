<script lang="ts">
  ...
  async function saveSurgery() {
    ...
    try {
      ...
      const surgeriesRef = collection(doc(db, 'patients', patientId), 'surgeries');
      const existing = await getDocs(surgeriesRef);
      const surgeryNumber = existing.size + 1;

      await addDoc(surgeriesRef, {
        surgeryNumber,
        hospitalTown,
        hospitalCountry,
        hospitalDepartment,
